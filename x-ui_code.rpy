...
init 2 python:
    ...
    import re
    remove_tags_expr = re.compile('{nw}|{image=.*?}|{/image}|{color=.*?}|{/color}|{a=.*?}|{/a}|{font=.*?}|{/font}|{size=.*?}|{/size}')
    ...
    def widget_button(text, displayable, clicked=None, style='prefs_label', xsize=220, ysize=30, widgetyoffset=3, textxoffset=30, state='button', xpos=0, ypos=0):
        ...
        thisbutton_hover = LiveComposite((xsize, ysize), (0, widgetyoffset), displayable, (textxoffset, 0), texthover)
        if (xpos or ypos):
            ui.fixed(xpos=xpos, ypos=ypos)
        if (state) == ('disabled'):
            thisbutton_disabled = LiveComposite((xsize, ysize), (0, widgetyoffset), ib_disabled(displayable), (textxoffset, 0), textdisabled)
            ...
            if (state) == ('active'):
                ...
                thisbutton = LiveComposite((xsize, ysize), (0, widgetyoffset), ib_base(displayable), (textxoffset, 0), textbase)
    ...
    def custom_render_savefile(index, name, filename, extra_info, screenshot, mtime, newest, clickable, has_delete, **positions):
        ...
        ui.vbox()
        ui.hbox()
        ui.button(style=style.file_picker_entry[index], clicked=clicked, **positions)
        ...
        if newest:
            headstyle_o = '{b}'
            headstyle_c = '{/b}'
        s = (config.file_entry_format) % (dict(time=((headstyle_o) + (renpy.time.strftime(config.time_format, renpy.time.localtime(mtime)))) + (headstyle_c), save_name=scene_name, playtime=playtime))
    ...
    def custom_file_picker(selected, save, mode='manual', background=None):
        saved_games = renpy.list_saved_games(regexp='(auto-|quick-)?[0-9]+')
        newest = None
        newest_mtime = None
        newest_auto = None
        ...
        for (fn, extra_info, screenshot, mtime) in saved_games:
            ...
            if (mtime) > (newest_mtime):
                ...
                if (is_autosave(fn) and (mtime) > (newest_automtime)):
                    newest_auto = fn
        ...
        while True:
            ...
            if not (persistent.fpicker_yoffset):
                ...
                if ((persistent.fpicker_yoffset) > ((auto_offset) * ((len(displayed_slots)) - (5))) or (persistent.fpicker_yoffset) == (-(1))):
            ...
            ui.close()
            ui.close()
            ui.vbox(xpos=600, yalign=0.49299999999999999, box_spacing=2)
            if scrollable:
    ...
    class customSliderPreference(object):
        ...
        def render_preference(self, thisxpos=0, thisypos=0):
            ...
            def changed(v):
                self.set(v)
            ui.hbox()
            ui.bar(self.range, self.get(), changed=changed, style=style.prefs_slider[self.name])
            ui.null(width=15)
    ...
    class customVolumePreference(object):
        ...
        def render_preference(self, thisxpos=0, thisypos=0):
            ...
            ui.hbox()
            ui.bar(self.steps, self.get_slider_position(), changed=changed, style=style.prefs_volume_slider[self.name])
            ui.null(width=15)
            layout.label(self.name, 'prefs')
            ui.close()
    ...
    class GalleryGridLayout(object):
        ...
        def __call__(self, imagenum, image_count):
            ...
            (ox, oy) = self.offsets
            return dict(xpos=(ulx) + (((imagenum) % (cols)) * (ox)), ypos=(uly) + (((imagenum) // (cols)) * (oy)))
        ...
        def render(self, i, pos):
            ...
            if not (self.check_unlock()):
                ...
                if (insensitive) is not (None):
                    ...
                    return None
            if self.hover:
                ui.imagebutton(self.idle, self.hover, clicked=ui.returns(('button', i)), **props)
            else:
                ui.fixed(**props)
                ui.image(self.idle)
                ui.imagebutton(self.gallery.idle_border, self.gallery.hover_border, clicked=ui.returns(('button', i)))
                ui.close()
        def show(self):
            all_prior = True
            im_sort = []
    ...
    class Gallery(object):
        ...
        def navigation(self, page_name, page_num, pages, currentpage=0):
            ...
            if (len(self.pages)) > (1):
                ...
                layout.button('<', 'gallery_nav', selected=False, clicked=myclick)
            for (i, p) in enumerate(self.pages):
                ...
                if (i) == (page_num):
                    mysel = True
                    myclick = False
                layout.button(p.name, 'gallery_nav', selected=mysel, clicked=myclick)
            if (len(self.pages)) > (1):
                myclick = ui.returns(('page', (currentpage) + (1)))
        ...
        def locked_image(self, num, total):
            ...
            ui.text((_(displayStrings.gallery_singlelocked)) % (((num) + (1), total)), xalign=0.5, yalign=0.69999999999999996, style='prefs_label')
        ...
        def autobutton(self, in_images):
            ...
            button_base = LiveComposite((110, 85), (0, 0), ib_base('ui/bt-cg-locked.png'), (5, 5), im.MatrixColor(thumbnail, im.matrix.desaturate()))
            button_hover = LiveComposite((110, 85), (0, 0), 'ui/bt-cg-locked.png', (5, 5), thumbnail)
    ...
    def extra_button(text, in_displayable, clicked=None, style='prefs_label', state='button'):
        ...
        if (state) == ('return'):
            textbase = LiveComposite((100, 30), (0, 3), ib_base('ui/bt-return.png'), (30, 0), textbase)
            texthover = LiveComposite((100, 30), (0, 3), 'ui/bt-return.png', (30, 0), texthover)
        if (state) == ('disabled'):
    ...
    def refresh_label(a, b):
        disp = Text(ss_desc, xalign=0.5, yalign=0.97999999999999998, size=18, **displayStrings.styleoverrides)
        return (disp, 0.050000000000000003)
    def ss_unhovered():
        global ss_desc
        ss_desc = ''
    ...
    class drugsDisp(renpy.Displayable):
        def __init__(self, width, height):
            ...
            self.length = 22.0
            self.fadeintime = 1.0
            self.framelength = 0.040000000000000001
            self.width = width
            self.height = height
            self.make_words()
        def make_words(self):
    ...
    def custom_movie_cutscene(filename, delay=None, loops=0, stop_music=True):
        ...
        if renpy.game.log.forward:
            ...
            roll_forward = None
        rv = renpy.ui.interact(suppress_overlay=True, suppress_underlay=True, show_mouse=False, roll_forward=roll_forward)
        if not ((filename) in (persistent.seen_videos)):
