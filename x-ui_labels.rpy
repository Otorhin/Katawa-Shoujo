...
label splashscreen:
    ...
    $ renpy.pause(0.20000000000000001)
...
label text_history_gm:
    python:
        ...
        if (not (current_line) and (len(readback_buffer)) == (0)):
            ...
            if (current_line and (len(readback_buffer)) == (0)):
                lines_to_show = [current_line]
            else:
                if (current_line and (current_line) != (readback_buffer[-(1)])):
...
label prefs_screen:
    ...
    python:
        ...
        if prefs_looped:
            ...
            if ((mm_context() or entered_from_game) and not coming_from_prefs_sub):
                renpy.transition(config.main_game_transition)
            else:
                renpy.transition(config.intra_transition)
        if mm_context():
...
label language_screen:
    python:
        ...
        ui.close()
        if not (mm_context()):
            ui.text(displayStrings.config_language_restart_note, text_align=0.5, xalign=0.5, yalign=0.97999999999999998, size=18)
...
label load_screen_loop:
    ...
    python:
        if mm_context():
            mybackground = LiveComposite((800, 600), (0, 0), style.mm_static.background, (0, 0), style.gm_root.background, (150, 100), 'ui/bg-config.png')
        else:
            mybackground = LiveComposite((800, 600), (0, 0), style.gm_root.background, (150, 100), 'ui/bg-config.png')
        if entered_from_game:
...
label extra_menu:
    ...
    python:
        ...
        if ((len(persistent.seen_videos)) >= (1) or has_devlvl()):
            videostate = 'button'
        ui.image(LiveComposite((800, 600), (0, 0), style.mm_static.background, (0, 0), style.gm_root.background, (150, 100), 'ui/bg-config.png'))
        ui.vbox(xpos=180, ypos=120, background=None)
...
label video_menu:
    ...
    python:
        ...
        for this_video in displayStrings.videos:
            ...
            button_base = LiveComposite((110, 85), (0, 0), ib_base('ui/bt-cg-locked.png'), (5, 5), im.MatrixColor(this_tn, im.matrix.desaturate()))
            button_hover = LiveComposite((110, 85), (0, 0), 'ui/bt-cg-locked.png', (5, 5), this_tn)
            if ((this_video[1]) in (persistent.seen_videos) or has_devlvl()):
                ui.imagebutton(button_base, button_hover, clicked=ui.returns(this_video[1]))
            else:
...
label cg_gallery:
    ...
    python:
        ...
        mygallery.background = LiveComposite((800, 600), (0, 0), style.mm_static.background, (0, 0), style.gm_root.background, (150, 100), 'ui/bg-config.png')
...
label softquit:
    ...
    python:
        if (_preferences.transitions) != (0):
            ...
            ui.image(Solid('#000'))
            renpy.pause(0.69999999999999996)
        renpy.jump('_quit')
...
label scene_deleted(is_end=False):
    ...
    $ renpy.music.set_volume(0.050000000000000003, 2.0, channel='music')
...
init 5 python:
    ...
    def skipcredits_overlay():
        ...
        ui.textbutton('{color=#000}Skip{/color}', clicked=clicked, xpos=795, xanchor=1.0, ypos=595, yanchor=1.0, xpadding=6, background=RoundRect((48, 48, 48, 255), 6), hover_background=RoundRect((64, 64, 64, 255), 6), color='#000')
...
label credits(c_movie=False):
    ...
    python:
        ...
        if c_movie:
            textoffset += 160
        credits_final = LiveComposite((800, totalsize), (313, 233), 'ui/cred_logo.png', (textoffset, (600) + (buffer)), creditstext_disp, (311, ((totalsize) - (600)) + (238)), 'ui/4lsl-small.png')
    ...
    show expression Text(u"©MMXII Four Leaf Studios", text_align=0.5, size=15)  at Position(xalign=0.5, yalign=0.576) as copyright
    with Dissolve(2.0)
...
label act_op(this_video):
    ...
    python:
        ...
        renpy.pause(0.10000000000000001)
        act = this_video[6:7]
        char = this_video[8:-(4)]
...
label path_end(character="", is_good=False):
    ...
    if not automode:
        if end_id == "bad":
            ...
            if renpy.loadable("video/credits_"+character+".mp4"):
                $ myvid = (('video/credits_') + (character)) + ('.mp4')
            else:
                $ myvid = False
            scene white
            with Dissolve(4.0)
